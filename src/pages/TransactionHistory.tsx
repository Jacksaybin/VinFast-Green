...
const TransactionHistory: React.FC = () => {
  ...
  const getUserTransactions = useWalletStore((s) => s.getUserTransactions);
  const [filter, setFilter] = useState<'all' | TransactionType>('all');

  const transactions = getUserTransactions(user.id).filter(
            ...
            {transactions.map((tx) => (
              <div key={tx.id} className="bg-white rounded-xl shadow-sm p-4">
                <div className="flex items-start justify-between mb-2">
                  <div>
                    <p className="font-semibold text-gray-900">{TYPE_LABELS[tx.type]}</p>
