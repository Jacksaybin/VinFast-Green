  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="px-4 pb-20">
        ...
        {loading ? (
          <div className="text-center py-10 text-gray-400 text-sm">Đang tải tin tức...</div>
        ) : (
          <>
            {/* Featured News */}
            <div className="mb-6">
              ...
              <div className="space-y-4">
                {featuredNews.slice(0, 2).map((newsItem) => (
                  <button
                    ...
                    className="w-full text-left bg-white rounded-xl shadow-sm overflow-hidden hover:shadow transition-shadow"
                  >
                    <div className="relative h-48">
                      <img src={imageOf(newsItem)} alt={newsItem.title} className="w-full h-full object-cover" />
                      <div className="absolute top-4 left-4">
                        <span className="bg-red-500 text-white px-2 py-1 rounded-full text-xs font-medium">Nổi bật</span>
                      </div>
                    </div>
                    <div className="p-4">
                      ...
                      <p className="text-sm text-gray-600 mb-3 line-clamp-2">{newsItem.summary}</p>
            ...
            <div className="mb-6">
              ...
              {filteredNews.length === 0 ? (
                <div className="text-center py-10 text-gray-400 text-sm">Không có bài viết nào</div>
              ) : (
                <div className="space-y-4">
                  {filteredNews.map((newsItem) => (
                    <button
                      key={newsItem.id}
                      onClick={() => navigate(`/news/${newsItem.slug}`)}
                      className="w-full text-left bg-white rounded-xl shadow-sm p-4 hover:shadow transition-shadow"
                    >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center space-x-2 mb-2">
                            <span className={`px-2 py-1 rounded text-xs font-medium ${
                              newsItem.category === 'vgreen' ? 'bg-green-100 text-green-600' :
                              newsItem.category === 'market' ? 'bg-blue-100 text-blue-600' :
                              'bg-purple-100 text-purple-600'
                            }`}>
                              {newsItem.category === 'vgreen' ? 'V-GREEN' : newsItem.category === 'market' ? 'Thị trường' : 'Chính sách'}
                            </span>
                            {newsItem.isFeatured && (
                              <span className="bg-yellow-100 text-yellow-600 px-2 py-1 rounded text-xs font-medium">Nổi bật</span>
                            )}
                          </div>
                          <h3 className="font-semibold text-gray-900 mb-1 line-clamp-1">{newsItem.title}</h3>
                          <p className="text-sm text-gray-600 mb-2 line-clamp-2">{newsItem.summary}</p>
