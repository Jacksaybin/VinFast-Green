...
const PackageListCard: React.FC<PackageListCardProps> = ({ pkg, onInvest, onViewDetail }) => {
  const categoryColors = {
    ...
    basic: 'from-brand-primary-500 to-green-600',
  };
        ...
        <div className="flex-1 p-4">
          ...
          <div className="grid grid-cols-3 gap-2 text-xs mb-3">
            <div className="bg-background rounded-lg p-2 text-center">
              ...
              <div className="font-semibold text-foreground">{formatCurrency(pkg.investmentAmount)}</div>
            </div>
            <div className="bg-background rounded-lg p-2 text-center">
              <div className="text-muted-foreground flex items-center justify-center gap-1">
                <Clock className="w-3 h-3" /> Kỳ hạn
              </div>
              <div className="font-semibold text-foreground">{pkg.investmentPeriod} ngày</div>
            </div>
            <div className="bg-background rounded-lg p-2 text-center">
              <div className="text-muted-foreground flex items-center justify-center gap-1">
                <Zap className="w-3 h-3" /> Quy mô
              </div>
              <div className="font-semibold text-foreground">{formatCurrency(pkg.projectScale)}</div>
            </div>
          </div>
